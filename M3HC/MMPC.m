% covMat: NxM matrix with N samples and M variables
...
function [selectedVars] = MMPC(covMat, target, maxK, threshold, testFunc, testParams, data)
nvars = size(covMat,2);
...
while any(remainingVars)
    [curvar, curpvalue, remainingVars, pvalues, stats] = findMaxMinAssociation(covMat, target, maxK, selectedVars, remainingVars, pvalues, stats, testFunc, testParams, data, threshold);
...
for i = 1:length(idx)
   ...
   curpvalue = findMinAssociation(covMat, target, curvar, maxK, curSelectedVars, pvalues, stats, testFunc, testParams, data, threshold);
   if(curpvalue > threshold)
       toremove(idx(i)) = true;
   else
...
function  [bestvar, bestpvalue, remainingVars, pvalues, stats] = findMaxMinAssociation(covMat, target, maxK, selectedVars, remainingVars, pvalues, stats, testFunc, testParams, data, threshold)
...
for var = 1:length(remainingVars)
    if(remainingVars(var))
        [curpvalue, curstat, pvalues, stats] = findMinAssociation(covMat, target, var, maxK, selectedVars, pvalues, stats, testFunc, testParams, data, threshold);
...
function [maxpvalue, minstat, pvalues, stats] = findMinAssociation(covMat, target, var, maxK, selectedVars, pvalues, stats, testFunc, testParams, data, threshold)
...
if(maxK == 0)
    [pvalue, stat] = testFunc(target, var, [], covMat, testParams);
...
%             stat=stat2;
    ...
    % Case k = 1
    [pvalue, stat] = testFunc(target, var, selectedVars(end), covMat, testParams);
...
%               stat=stat2;
    ...
    for k = 2:maxK
        ...
        for i = 1:size(subsets,1)
            [pvalue, stat] = testFunc(target, var, [subsets(i,:) selectedVars(end)], covMat, testParams);
